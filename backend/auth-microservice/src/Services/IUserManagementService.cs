...
{
    Task<UserResponseDto?> GetAsync(Guid id, CancellationToken token);
    Task<IReadOnlyList<UserResponseDto>> SearchAsync(string? search, CancellationToken token);
    ...
    Task<bool> ActivateAsync(Guid id, CancellationToken token);
}
