...
var app=builder.Build(); using(var scope=app.Services.CreateScope()){ await scope.ServiceProvider.GetRequiredService<MoviesDbContext>().Database.MigrateAsync(); } app.UseCors(); app.MapGet("/api/movies", async (string? search, IMovieService service, CancellationToken token)=>Results.Ok(await service.GetActiveAsync(search,token))); app.MapPost("/api/movies", async (Movies.Domain.CreateMovieRequestDto request, IMovieService service, CancellationToken token)=>Results.Created("/api/movies",await service.CreateAsync(request,token))); app.Run();
